...
当前 v0.7.0 的行为仍是「扫地机 + 打磨工」,但所有冲突统一按 `SKILL.md` 的「冲突仲裁顺序」六级处理:
...
| 方面 | 英文 humanizer | qu-ai-wei（当前 v0.7.0） |
|---|---|---|
...
执行顺序、语体矩阵和规则索引看 [`SKILL.md`](./SKILL.md)；完整规则、触发条件和“原文/改后”示例在 [`references/patterns.md`](./references/patterns.md)。README 这里只保留目录级速览，方便先判断是否覆盖你的场景。
...
- 全规则与触发细节：[`references/patterns.md`](./references/patterns.md)
