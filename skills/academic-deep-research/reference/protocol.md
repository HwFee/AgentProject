...
1. `web_search` broad query (count=20).
2. Synthesize patterns and initial hypotheses.
3. Identify gaps and contradictions.

### Cycle 2: Deep Investigation
1. Targeted `web_search` on gaps.
2. `web_fetch` primary sources.
3. Re-evaluate assumptions; integrate new evidence.
...
Per theme:
1. Start with `web_search` for landscape.
2. Analyze and identify gaps.
3. `web_fetch` primary sources.
4. Analyze, then repeat with targeted searches.
